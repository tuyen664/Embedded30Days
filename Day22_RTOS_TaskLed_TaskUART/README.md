...
- Scheduler : Quản lý CPU và phân chia thời gian cho 2 task

## main() 
**1. SystemInit();**
- Hàm này làm 2 việc quan trọng là :
...
- Scheduler = "người điều phối CPU" trong hệ điều hành RTOS
- Nó quyết định task nào được phép chạy trong từng thời điểm.
...
- Trong FreeRTOS thì  1 time slice = 1 tick (ta cso thể quay đổi 1 tick ra các thời gian khác nhau)
- Nếu có nhiều task cùng priority và đang Ready → MỖI TICK sẽ chuyển sang task tiếp theo.
...
**3.3**
- 128 – **stack size** của task , đơn vị stack là word (4 byte) : 128 x 4 = 512 bytes stack
...
```
**3.5**
- **priority = 1** : Đây là độ ưu tiên của task.(Priority = 1 > priority 0)
