...
**1. Cấu hình TIM2**
  - Bật Clock cho TIM2 : RCC->APB1ENR |= (1<<0); (Bật clock cho TIM2 ,TIM2 nằm trên bus APB1 bit 0;)
  ...
  - Cho phép TIMER sinh ra ngắt (bit 0  là UIE- update interrupt enable) : TIM2->DIER |= (1<<0);
...
**2. Khi nhấn nút Reset**
  ...
  - Xóa cờ ngắt sau khi xử lý xong để lần sau timer tràn tiếp thì mới kích hoạt lại.
  + TIM2->SR &= ~(1 << 0);      // Clear flag
  + UIF không tự xóa khi CPU đọc, phải ghi 0 vào bit này để clear cờ theo cơ chế ‘write 0 to clear
