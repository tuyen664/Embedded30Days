...
static void Task_Button(void *pvParameters)
{
    ...
    for (;;)
    {
        ...
        if (prev == 1 && curr == 0)
        {
            xQueueSend(xQueueButton, &msg, 0); // 0 - not wait
        }
        prev = curr;
        vTaskDelay(debounce);
    }
...
**4. Lưu ý quan trọng**
- Task chạy từ đầu hàm CHỈ 1 LẦN → sau đó mỗi lần scheduler quay lại, nó chỉ chạy tiếp cái chỗ nó dừng.
...
if (xSemaphoreTake(xUART_Semaphore, portMAX_DELAY) == pdTRUE)
```
- UART là resource chung dùng bởi 2 task : Task_Button_UART và Task_Semaphore
...
- Việc toggle LED ở đây OK nhưng sẽ xung đột với Task_LED nếu dùng cùng 1 LED
