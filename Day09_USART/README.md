...
**1. Cấu hình USART**
  - Bật clock cho GPIOA(bit 2) VÀ USART (bit 14) trong thanh ghi RCC->APB2ENR
  - Cấu hình PA9 làm TX (chân gửi dữ liệu ra Tera Term) , PA10 làm RX (chân nhận dữ liệu từ ngoài vào)
  + PA9 : Alternate Function Push-Pull , 50MHz (1011) : GPIOA->CRH |=  (0x0B<<4);    
  + PA10 : Input Floating mode (0100) → GPIOA->CRH |= (0x04<<8);
  - cấu hình Baudrate (tốc độ truyền mong muốn ) : USART1->BRR = 0x1D4C;    
  + hoặc có thế trực tiếp USART1->BRR = 72000000/9600; // tốc độ truyền 9600
 ...
 **2.1 void USART1_SendChar(char c)**
  ...
  + TXE = 1: Thanh ghi truyền DR trống → sẵn sàng nhận dữ liệu mới để gửi.
