...
- Cả hai dùng Open-Drain + pull-up, SCL clock tốc độ 100kHz, SDA truyền bit theo SCL.
- Start/Stop condition quyết định khi SDA đổi trạng thái lúc SCL cao 

- APB1ENR bật clock cho I2C1
- PB6 và PB7 là chân I2C. Chúng ta cần cấu hình Alternate Function Open-Drain (AF-OD) với tốc độ 50MHz.
- GPIOB->CRL |=  (0xFU << 24) | (0xFU << 28); // PB6, PB7 AF-OD 50MHz
- Open-Drain nghĩa là chân chỉ kéo xuống đất, I2C dùng chung pull-up resistor
- Lưu ý: 
...
- I2C1->CR1 = 0;            // SWRST = 0 -> nhả nút reset
...
**1.4 Cấu hình tốc độ I2C (100 kHz)**
- I2C1->CCR = 180;
- CCR = Clock Control Register, xác định độ dài xung SCL
- Công thức: CCR = PCLK1 / (2 * Fscl)
- PCLK1 = 36MHz, Fscl = 100kHz → CCR = 36,000,000 / (2*100,000) = 180
...
- Công thức: TRISE = (PCLK1 * 1us) + 1 -> 36MHz * 1us + 1 = 36 +1 = 37
...
**3. Gửi địa chỉ slave**
- Gửi địa chỉ I2C của thiết bị slave (vd  LCD) : I2C1->DR = addr; -> Tự động clear Start bit
- Xác nhận địa chỉ đã gửi thành công và slave ack : while (!(I2C1->SR1 & (1U << 1))). Nếu slave không ack, ta gửi STOP và thoát hàm
- if (I2C1->SR1 & (1U << 10)) // slave không phản hồi bit 10 sẽ được set lên 1 , SR1 bit10 = AF (Acknowledge Failure) , khi đó ta cần clear flag ADDR và báo lỗi .
...
- LCD chỉ nhận 4-bit/lần nên chúng ta làm : upper: lấy 4 bit cao , lower: dời 4 bit thấp lên để đưa vào D4..D7
...
- SDA và SCL phải có pull-up 4.7kΩ -> Nhiều board không hề có pull-up, nên code đúng nhưng thiết bị vẫn không chạy (thư viện bên trong đã cấu hình sãn #define __IO volatile )
-> Không có pull-up thì I2C = không chạy
