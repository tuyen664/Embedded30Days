...
## Tổng quát 
 ...
 - STM32F1 dùng ADC 12-bit đo điện áp tại chân PAx : Giá trị 0…4095 tương ứng 0…Vref
...
### 1. Cấu hình ADC
 ...
 - Chia clock ADC (ADCPRE): ``` RCC->CFGR |= (0x2U << 14); ``` ADCPRE = 10 → PCLK2/6 → 72/6 = 12 MHz (≤ ~14 MHz)
   
    Dùng tốc độ cao hơn vẫn chạy , độ chính xác giảm

 - Cấu hình các chân PA0–PA2 là  analog input (chỉ cần reset về 0)
 - ADC_CH0 = 0, ADC_CH1 = 1, ADC_CH2 = 2

 - SMPR2 (Sample time register) : Thời gian lấy mẫu của ADC
   
    ```ADC1->SMPR2 |=  (0x7U << (3 * ADC_CH1)) ;``` // Mỗi kênh dùng 3 bit để chọn sample cycles
   
    0x7 = 111 → chọn 239.5 ADC cycles (lâu nhất, phù hợp nguồn có R lớn)
...
### 2. Cấu hình ADC khi dùng DMA
 ...
 - ```ADC1->CR1 |= (1U << 8); ```          // SCAN = 1
    ...
    SCAN mode = 1 → ADC đọc nhiều kênh liên tiếp theo thứ tự định nghĩa trong SQRx
   
    SCAN = 0 → chỉ đọc 1 kênh duy nhất lặp đi lặp lại

 - ```ADC1->CR2 |= (1U << 1); // CONT = 1```
   
    ADC sẽ liên tục chuyển đổi, không cần SWSTART mỗi lần 
   
 - ```ADC1->CR2 |= (1U << 8);  DMA = 1```
   
     Cho phép ADC tạo DMA request; DMA chỉ thực hiện copy khi ADC conversion hoàn tất (EOC)
 ...
 - Chọn nguồn kích hoạt ADC là SWSTART :  ```ADC1->CR2 |=  (0x7U << 17);``` 
 - ADC1->CR2 &= ~(1U<<20); // EXTTRIG = 0 (không dùng ngoại vi) 
 - Vì dùng continuous mode, ADC sẽ tự quét liên tục, nên SWSTART chỉ cần gọi 1 lần 
...
### 3. Cấu hình DMA1 channel1
 ...
 - DMA giúp ADC tự động đẩy kết quả vào vùng nhớ mà không tốn CPU
 - Enable DMA1 clock : ```RCC->AHBENR |= (1U << 0);```
 - Tắt channel trước config: DMA1_Channel1->CCR &= ~(1U << 0); // EN = 0 

 - DMA1_Channel1->CPAR = (uint32_t)&ADC1->DR;
   
    -> Địa chỉ ngoại vi (Peripheral Address): nơi DMA tự động đọc dữ liệu khi ADC hoàn thành 1 conversion
...
### 5. Vòng Lặp chính
 ...
 - Phải sắp xếp thứ thự chạy các hàm hợp lý , để tránh DMA đọc nhầm dữ liệu rác
 - Cấu hình UART trước
...
**Tóm Tắt quá trình hoạt động**
 ...
 - Bật bit DMA = 1, ADC cho phép DMA hoạt động, nhưng DMA chỉ thực sự chuyển dữ liệu khi ADC có dữ liệu trong DR
 ...
 - Trong vòng while(1) CPU chỉ việc đọc adc_buf[], đổi sang Volt theo công thức :  V = ADC_value × 3.3​ / 4095
