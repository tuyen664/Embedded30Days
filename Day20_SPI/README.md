...
-  Cấu hình SPI1 trên STM32 , bao gồm:
...
## 2. SPI1_Init()
- Bật clock cho GPIOA , AF , SPIEN, DMA1EN
...
- SPI_CR1_SSM : cho phép MCU tự quản lý chân NSS
   ...
   Nếu Không dùng SSM (hardware NSS): PA4 được SPI điều khiển → chỉ 1 slave → rất hạn chế
...
- SPI_CR1_SPE : Bật SPI
     ...
     (0x3 << 3) = BaudRate = Fpclk / 16 -> Baudrate phụ thuộc APB2 clock
...
- RXNE = 1 nghĩa là có dữ liệu cũ trong RX buffer. Nếu không đọc, byte mới khi gửi sẽ gây Overrun (OVR)
   ...
   Dùng dummy = SPI1->DR; để “hút” dữ liệu cũ
...
- TXE=1 không đảm bảo byte trước đó đã clock ra ngoài
   ...
   Ta cần check BSY , BSY=1 nghĩa là SPI đang truyền dữ liệu trước đó
...
- là hàm truyền một mảng dữ liệu qua SPI1 tới slave , và nhận dữ liệu trả về theo kiểu polling
...
- CTCIF2 = Clear Transfer Complete Flag for channel 2 , ghi 1 vào để CLEAR
...
- Khai báo volatile để compiler không tối ưu, vì nó thay đổi trong ISR
- Nếu không khai báo volatile, compiler có thể “giả sử” giá trị không đổi trong main loop → bug rất khó nhận ra
  
**4. Check TEIF**

- TEIF = transfer error , báo lỗi transfer, không phải lỗi MCU
- TEIE = bật/tắt khả năng nhảy vào ISR khi TEIF = 1 -> phòng trường hợp bị lỗi thì NVIC kịp thời phát hiện 
- Trong ngắt chỉ báo lỗi , ví dụ thêm dmaRxErrorFlag = 1; làm việc nặng như thông báo ,... thì ở main hoặc task
...
```
- DMA1 Channel 3 được dùng truyền dữ liệu (TX) của SPI1
- Khi DMA TX hoàn tất truyền buffer → cờ TCIF3 set lên 1 → NVIC gọi handler này
- Hàm này xử lý các việc : 
   ...
   KHÔNG cần set flag báo main loop, vì main loop chỉ quan tâm RX DMA

**Lưu ý**

- ISR chỉ có check cờ + clear cờ → nhanh → không cần disable NVIC
- Nếu ISR dài → cần NVIC_DisableIRQ để tránh race condition với các ngắt khác
- TCIF = transfer complete 
- TEIF = transfer error → báo lỗi
...
- Timeout = “safety net” , tốt hơn dùng semaphore/RTOS event
...
- Dữ liệu của SPI không hợp lệ liên tục, mà chỉ hợp lệ tại cạnh lên hoặc cạnh xuống của SCK
...
- CPU bị block trong lúc polling → nếu dùng RTOS
