...
- Nếu ta gọi API normal trong ISR → crash hoặc lỗi bất thường
...
- ISR Give semaphore → Task_TimerHandler READY → priority cao nhất → cần chạy NGAY **không chờ tick**
...
**4. Mutex khác gì Binary Semaphore?**
...
- Scheduler phải duyệt priority và chọn trong các task đã READY để chạy
...
```
- Nhả semaphore , Nếu task TimerHandler priority cao hơn task hiện tại → yield ngay
...
- API kiểm tra: có task unblock nào priority cao hơn đang chạy không
   ...
   Sau đó gọi portYIELD_FROM_ISR(xHigherPriorityTaskWoken)
